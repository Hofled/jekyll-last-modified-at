...
describe 'Last Modified At Tag' do
  context 'A committed post file' do
    ...
    it 'has last revised date' do
      setup('1984-03-06-last-modified-date.md', 'last_modified_at.html')
      expect(@post.output).to match(/Article last updated on 03-Jan-14/)
    end

    it 'passes along last revised date format' do
      setup('1984-03-06-last-modified-date-with-format.md', 'last_modified_at_with_format.html')
  ...
  context 'An uncommitted post file' do
    before(:all) do
      cheater_file = '1984-03-06-last-modified-date.md'
      uncommitted_file = '1992-09-11-last-modified-date.md'
